    ...
    async def generate():
        try:
            chat = LlmChat(
                ...
                system_message=get_system_message(session.job_title)
...
async def interview_chat(request: ChatRequest):
    ...
    is_final = question_count >= 6
    if is_final:
        user_msg_text = f"[FINAL_EVALUATION]\n\nCandidate's final answer: {request.user_message}"
    else:
        user_msg_text = request.user_message
    ...
    async def generate():
        try:
            ...
            logger.error(f"Error in interview_chat: {e}")
            yield f"data: {json.dumps({'error': str(e)})}\n\n"
